...
if ($_SERVER['REQUEST_METHOD']=='POST') {
	...
	if (!isset($errors)) {
		try {
			...
			$invoiceline_id = DB::insert('INSERT INTO `invoicelines` (`tenant_id`, `customer_id`, `type`, `name`, `subtotal`, `vat`, `vat_percentage`, `total`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', $_SESSION['user']['tenant_id'], $data['deliveries']['customer_id'], 'delivery', $data['deliveries']['name'], $data['deliveries']['subtotal'], ($total - $data['deliveries']['subtotal']), $data['deliveries']['vat_percentage'], $total);
