...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($vat_reverse_charge) {
        $data['hours']['vat_percentage'] = null;
    }
    ...
    if (!isset($errors)) {
        try {
            ...
            if ($data['hours']['vat_percentage']) {
                $total = $subtotal * ((100 + $data['hours']['vat_percentage']) / 100);
            } else {
                $total = $subtotal;
            }
            ...
            $optional = DB::update('UPDATE `invoicelines` SET `customer_id`=?, `name`=?, `subtotal`=?, `vat`=?, `vat_percentage`=?, `total`=? WHERE `tenant_id` = ? AND `id` = ? AND `invoice_id` IS NULL', $data['hours']['customer_id'], $data['hours']['name'], $subtotal, ($total - $subtotal), $data['hours']['vat_percentage'], $total, $_SESSION['user']['tenant_id'], $hour['hours']['invoiceline_id']);
