...
if ($_SERVER['REQUEST_METHOD']=='POST') {
	...
	if (!isset($errors)) {
		try {
			...
			$invoiceline_id = DB::insert('INSERT INTO `invoicelines` (`tenant_id`, `customer_id`, `type`, `name`, `subtotal`, `vat`,`vat_percentage`,`total`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', $_SESSION['user']['tenant_id'], $data['hours']['customer_id'], 'hours', $data['hours']['name'], $subtotal, ($total - $subtotal), $data['hours']['vat_percentage'], $total);
