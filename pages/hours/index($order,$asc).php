...
if($order == 'customer') $data = DB::select('select * from `hours`,`customers` WHERE `hours`.`tenant_id` = ? AND `hours`.customer_id = `customers`.id order by `customers`.name '.$asc.' limit 200', $_SESSION['user']['tenant_id']);
