...
$hours = DB::select('select * FROM `hours` WHERE invoiceline_id IS NULL AND hourly_fee IS NOT NULL AND `customer_id` = ? AND `tenant_id` = ?', $customer_id, $_SESSION['user']['tenant_id']);
...
foreach ($hours as $row) {
	...
	if($tax_percentage) $total = $subtotal*((100+$tax_percentage)/100); 
	else $total = $subtotal;
...
if ($_SERVER['REQUEST_METHOD']=='POST') {
	...
	if (!isset($errors)) {
		...
		try {
			...
			//begin();
			$invoice_id = DB::insert('INSERT INTO `invoices` (`tenant_id`, `number`, `name`, `date`, `customer_id`, `subtotal`, `tax`, `total`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', $_SESSION['user']['tenant_id'], ($highest_invoice_number+1), $data['invoices']['name'], date('Y-m-d'), $customer_id, $sums['subtotal'], $sums['tax'], $sums['total']);
